...
/**
 * Generic base class for JPA {@link AttributeConverter}s that convert
 * domain attribute types to and from a single database column value.
 *
 * <p>Both conversion directions are supplied as constructor arguments, keeping
 * each concrete converter to a single-line constructor.  The two-function design
 * handles cases where the domain attribute's wrapped type differs from the
 * database column type (e.g. {@code MediaFileId} wraps {@code UUID} but is
 ...
 * }</pre>
 *
 * <p>JPA resolves the {@code autoApply} target type from the concrete type argument
 * {@code A} in the subclass declaration — type erasure does not affect this because
 * the type is known statically at the subclass level.
 *
...
public abstract class DomainAttributeConverter<A, T> implements AttributeConverter<A, T> {
    ...
    protected DomainAttributeConverter(final Function<T, A> fromDatabase, final Function<A, T> toDatabase) {
