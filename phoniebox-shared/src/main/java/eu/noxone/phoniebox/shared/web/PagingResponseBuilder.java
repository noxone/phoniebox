 ...
 *
 * <p>The response body is the plain content array; all pagination metadata is carried in standard
 * HTTP headers so callers do not need to unwrap an envelope:
 *
 * <ul>
 ...
 *   <li>{@code Link} — RFC 8288 navigation links: {@code first}, {@code last}, and conditionally
 *       {@code prev} / {@code next}
 * </ul>
 */
public final class PagingResponseBuilder {
  ...
  private static String buildLinkHeader(PageResponse<?> page, UriInfo uriInfo) {
    ...
    links.add(
        linkEntry(
            base.clone().replaceQueryParam("page", Math.max(0, page.totalPages() - 1)), "last"));
