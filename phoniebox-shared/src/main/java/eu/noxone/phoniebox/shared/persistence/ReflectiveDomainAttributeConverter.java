...
/**
 * Reflective base class for JPA converters where the domain attribute's wrapped type is the same as
 * the database column type (e.g. {@code DefaultDomainAttribute<String>} stored as {@code VARCHAR},
 * {@code DefaultDomainAttribute<Long>} stored as {@code BIGINT}).
 *
 ...
 * {@code A} via reflection (trying the primitive equivalent when {@code T} is a boxed numeric, so
 * that e.g. {@code FileSize.of(long)} is found for {@code T = Long}). {@link
 * #convertToDatabaseColumn} simply delegates to {@link DefaultDomainAttribute#getValue()}.
 *
 ...
 * from the database column type (e.g. {@code AbstractId} wraps {@code UUID} but is stored as {@code
 * String}; {@code UploadedAt} wraps {@code Instant} but is stored as an ISO-8601 {@code String}),
 * use {@link DomainAttributeConverter} with explicit {@code fromDatabase} and {@code toDatabase}
 * functions instead.
 *
        ...
        dbValue -> {
          try {
            ...
            throw new RuntimeException(
                "Cannot convert database value '" + dbValue + "' to " + domainType.getSimpleName(),
                e);
          }
        };
  }
    ...
    try {
      ...
      throw new IllegalStateException(
          domainType.getSimpleName()
              + " must declare a public static of("
              + dbType.getSimpleName()
              + ") factory method for reflective conversion",
          boxedNotFound);
    }
  }
