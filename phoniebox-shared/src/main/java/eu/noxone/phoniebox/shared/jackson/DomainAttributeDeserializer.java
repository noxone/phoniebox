 ...
 *
 * <p>Implements {@link ContextualDeserializer} so Jackson can specialise this deserializer for each
 * concrete attribute type it encounters. When Jackson resolves the deserializer for a field like
 * {@code MimeType mimeType}, it calls {@link #createContextual} with {@code MimeType.class} as the
 * target type. The returned specialised instance resolves the wrapped type {@code T} from the
 * generic supertype chain and locates the {@code of(T)} factory method by reflection. Subsequent
 * calls to {@link #deserialize} read the JSON value as {@code T} and invoke the factory to produce
 * the domain attribute.
 *
 * <p>Example — JSON {@code "audio/mpeg"} deserializes to {@code MimeType.of("audio/mpeg")}; JSON
 * {@code 1024} deserializes to {@code FileSize.of(1024L)}.
 *
...
public class DomainAttributeDeserializer extends StdDeserializer<DefaultDomainAttribute<?>>
  ...
  private DomainAttributeDeserializer(
      final Class<? extends DefaultDomainAttribute<?>> attributeType) {
    super(attributeType);
  ...
  @Override
  public DefaultDomainAttribute<?> deserialize(
      final JsonParser p, final DeserializationContext ctxt) throws IOException {
    if (factoryMethod == null) {
      throw new JsonMappingException(
          p, "DomainAttributeDeserializer used without type context — this is a bug");
    }
    ...
    try {
      ...
      throw new JsonMappingException(
          p, "Failed to construct " + handledType().getSimpleName() + " from JSON value", e);
    }
  }
  ...
  private static Class<?> resolveWrappedType(final Class<?> attributeClass) {
    ...
    throw new IllegalArgumentException(
        "Cannot resolve wrapped type for "
            + attributeClass.getName()
            + " — it does not extend DefaultDomainAttribute<T>");
  }

  private static Method resolveFactoryMethod(
      final Class<?> attributeClass, final Class<?> wrappedType) {
    try {
