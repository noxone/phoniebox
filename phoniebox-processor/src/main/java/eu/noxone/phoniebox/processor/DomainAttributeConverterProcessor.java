...
/**
 * Annotation processor that automatically generates JPA {@code @Converter} classes for concrete
 * {@code DefaultDomainAttribute<T>} subclasses whose wrapped type maps directly to a standard JDBC
 * column type.
 *
 * <h2>What gets generated</h2>
 *
 * <p>For every non-abstract class {@code Foo extends DefaultDomainAttribute<T>} found in a {@code
 * *.domain.model*} package where {@code T} is a known simple type ({@code String}, {@code Long},
 * {@code Integer}, …), the processor emits:
 *
 * <pre>
 ...
 *
 * <p>Types whose wrapped type is not a simple JDBC type (e.g. {@code UUID}, {@code Instant}) are
 * skipped. A {@code NOTE} diagnostic is emitted for each skipped type as a reminder to write a
 * manual converter.
 *
 * <h2>Activation</h2>
 *
 * <p>Declare {@code phoniebox-processor} as a {@code provided} dependency in any feature module.
 * javac discovers the processor automatically via the SPI entry in {@code
...
public class DomainAttributeConverterProcessor extends AbstractProcessor {
  ...
  /**
   * Wrapped types that map directly to a JDBC column type. The value is the unqualified boxed type
   * name used in the generated source. All other wrapped types (UUID, Instant, …) are skipped.
   */
  ...
  public boolean process(
    ...
    for (final Element element : roundEnv.getRootElements()) {
      ...
      final TypeMirror erased = processingEnv.getTypeUtils().erasure(typeElement.asType());
      ...
      if (boxedTypeName == null) {
        processingEnv
            ...
            .printMessage(
                ...
                "Skipping converter generation for "
                    + typeElement.getQualifiedName()
                    + ": wrapped type '"
                    + wrappedTypeName
                    + "' is not a simple JDBC type."
  ...
  /**
   * Walks the supertype chain until it finds {@code DefaultDomainAttribute<T>} and returns the
   * resolved type argument {@code T}. Returns {@code null} if the chain does not reach {@code
   * DefaultDomainAttribute}.
   */
  private TypeMirror findWrappedType(final TypeElement typeElement, final TypeElement target) {
  ...
  private void generateConverter(final TypeElement typeElement, final String boxedDbTypeName) {
    ...
    final String originalPackage = ((PackageElement) enclosing).getQualifiedName().toString();

    // eu.noxone.phoniebox.media.domain.model  →
    // eu.noxone.phoniebox.media.infrastructure.persistence
    ...
    try {
      ...
      try (final Writer writer = file.openWriter()) {
        writer.write(
            renderSource(infraPackage, simpleName, qualifiedName, boxedDbTypeName, converterName));
      }
      processingEnv
          .getMessager()
          .printMessage(
              Diagnostic.Kind.NOTE, "Generated " + infraPackage + "." + converterName, typeElement);
  ...
  private static String renderSource(
    ...
    return "package "
        ...
        + ";\n"
        + "import "
        + REFLECTIVE_CONVERTER
        + ";\n"
        + "import jakarta.persistence.Converter;\n"
        + "\n"
        + "/**\n"
        + " * Generated JPA converter for {@link "
        + simpleName
        + "}.\n"
        + " *\n"
