 ...
 * <ul>
 *   <li>{@code MEDIA_FILE} — reads bytes from the local file system via {@link FileStoragePort}</li>
 *   <li>{@code AUDIO_STREAM} — opens a live HTTP connection to the stream URL</li>
 * </ul>
 */
@ApplicationScoped
public class MediaFileAudioStreamAdapter implements AudioStreamPort {
    ...
    public MediaFileAudioStreamAdapter(final FileStoragePort storage,
                                       final AudioStreamRepository audioStreamRepository) {
    ...
    private InputStream openAudioStream(final UUID id) throws IOException {
        AudioStream stream = audioStreamRepository.findById(AudioStreamId.of(id))
