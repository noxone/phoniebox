...
/**
 * Verifies the HTTP headers produced by {@link
 * eu.noxone.phoniebox.shared.web.PagingResponseBuilder} through a live JAX-RS endpoint. Quarkus is
 * required here because {@code Response.ok()} needs a registered {@code RuntimeDelegate}.
 */
@QuarkusTest
class PagingResponseBuilderTest {
  ...
  void nextLink_referencesIncrementedPageNumber() {
    ...
    given().when().get("/api/media?page=1&size=10").then().header("Link", containsString("page=2"));
  }
  ...
  void prevLink_referencesDecrementedPageNumber() {
    ...
    given().when().get("/api/media?page=2&size=10").then().header("Link", containsString("page=1"));
  }
  ...
  void firstLink_alwaysReferencesPageZero() {
    ...
    given().when().get("/api/media?page=2&size=10").then().header("Link", containsString("page=0"));
  }

  // ── helpers ───────────────────────────────────────────────────────────────

  private static PageResponse<MediaFile> pagedResponse(
      final int page, final int size, final long total) {
  ...
  private static MediaFile sampleFile() {
    return MediaFile.create(
        new MediaFileMetadata(
            OriginalFileName.of("track.mp3"), MimeType.of("audio/mpeg"), FileSize.of(1024L)));
  }
}
