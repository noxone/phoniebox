...
public class MediaFile extends DefaultDomainEntity<MediaFileId> implements Playable {
    ...
    /**
     * Updates the editable tag fields (title, artist, album, genre) and sets {@link #updatedAt} to now.
     * Technical fields (duration, bitrate, sample rate, track number, year) are preserved if present.
     * Null values clear the corresponding tag.
     */
    public void updateTagMetadata(final TrackTitle title, final TrackArtist artist,
                                  final TrackAlbum album, final TrackGenre genre) {
        this.audioMetadata = audioMetadata != null
