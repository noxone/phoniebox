...
/**
 * Adapter that implements the {@link MediaFileRepository} port by delegating to {@link
 * MediaFilePanacheRepository}.
 *
 * <p>Because {@link MediaFile} is itself a JPA {@code @Entity}, no manual domain↔persistence
 * mapping is needed. Hibernate's dirty-checking mechanism automatically persists any field changes
 * made to a managed {@link MediaFile} instance when the surrounding transaction commits.
 *
 * <p>The separation from {@link MediaFilePanacheRepository} avoids method-signature conflicts:
 * Panache's {@code findById} returns the entity directly while the port declares it as {@code
 * Optional}; similarly Panache's {@code findAll} returns a {@code PanacheQuery} while the port
 * declares a plain {@code List}.
 */
@ApplicationScoped
