 ...
 *
 * <p>Orchestrates the {@link MediaFileRepository} (persistence) and the {@link FileStoragePort}
 * (binary storage) without knowing anything about HTTP, SQL, or the file system.
 */
@ApplicationScoped
public class MediaFileApplicationService
    implements UploadMediaFileUseCase,
        GetMediaFileUseCase,
  ...
  @Inject
  public MediaFileApplicationService(
      final MediaFileRepository repository,
  ...
  public MediaFile upload(final UploadMediaFileCommand command) {
    ...
    audioMetadataExtractor
        .extract(storage.resolve(mediaFile.getId()))
  ...
  public Optional<MediaFile> updateTags(final UpdateTagsCommand command) {
    return repository
        .findById(MediaFileId.of(command.id()))
        .map(
            file -> {
