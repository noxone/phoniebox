 ...
 *
 * <p>Implements {@link DomainAttribute} so it can be used as a field on {@link MediaFile} without
 * violating the entity-field rule. Each sub-value is itself a typed {@link DomainAttribute} whose
 * persistence conversion is handled by an {@code @AttributeConverter(autoApply = true)} in the
 * infrastructure layer.
 *
 * <p>Marked {@code @Embeddable} so JPA maps its three columns directly into the {@code media_files}
 * table alongside the entity's own columns. Fields are non-final and a no-arg constructor is
 * provided for JPA.
 */
@Embeddable
