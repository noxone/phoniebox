 ...
 * </pre>
 *
 * <p>The resource only speaks to the application-layer input ports. It has no knowledge of
...
public class MediaFileResource {
  ...
  public Response upload(@RestForm("file") final FileUpload file) {
    try (var content = new BufferedInputStream(Files.newInputStream(file.filePath()))) {
      final var command =
          new UploadMediaFileCommand(
              file.fileName(), resolveMimeType(file), Files.size(file.filePath()), content);
