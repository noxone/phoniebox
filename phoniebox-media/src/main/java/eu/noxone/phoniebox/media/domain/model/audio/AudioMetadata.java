 ...
 *
 * <p>All fields are nullable — technical fields (duration, bitrate, sample rate) come from the
 * audio stream header and are usually present; tag fields (title, artist, …) depend on whether the
 * file was tagged. If metadata extraction fails entirely, the {@code audioMetadata} field on {@link
 * MediaFile} is {@code null}.
 *
 * <p>Marked {@code @Embeddable} so Hibernate maps its columns directly into the {@code media_files}
 * table alongside the entity's own columns.
 */
@Embeddable
public class AudioMetadata implements DomainAttribute {
  ...
  public TrackBitrate getBitrate() {
    return bitrate;
  }

  public TrackSampleRate getSampleRate() {
    return sampleRate;
  }
  ...
  /**
   * Returns a new instance with updated tag fields; technical fields (duration, bitrate, sample
   * rate, track number, year) are preserved.
   */
  public AudioMetadata withTags(
      final TrackTitle title,
      final TrackArtist artist,
      final TrackAlbum album,
      final TrackGenre genre) {
  ...
  public int hashCode() {
    return Objects.hash(
        duration, bitrate, sampleRate, title, artist, album, trackNumber, year, genre);
  }
}
