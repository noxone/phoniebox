 ...
 *
 * <ul>
 *   <li>All concrete classes in {@code domain.model} must implement {@link DomainEntity} or {@link
 *       DomainAttribute}.
 *   <li>All instance fields declared on a {@link DomainEntity} must implement {@link
 *       DomainAttribute} — plain Java types are not permitted.
 * </ul>
 */
public final class OnionArchitectureRules {
  ...
  public static ArchRule applicationDoesNotDependOnInfrastructureOrWeb(final String basePackage) {
    return noClasses()
        ...
        .should()
        .dependOnClassesThat()
        .resideInAnyPackage(basePackage + ".infrastructure..", basePackage + ".web..")
        .as("Application must not depend on infrastructure or web")
        .allowEmptyShould(true);
  }

  /**
   * Web (REST) classes must not directly reach into the infrastructure layer. They must go through
   * application-layer input ports instead.
   */
  ...
  /**
   * Every concrete class in the {@code domain.model} package must implement either {@link
   * DomainEntity} or {@link DomainAttribute}.
   *
   * <p>This prevents plain POJOs or data-bags from entering the domain model without being tagged
  ...
  public static ArchRule domainModelClassesMustBeEntityOrAttribute(final String basePackage) {
    return classes()
        ...
        .should()
        .beAssignableTo(DomainEntity.class)
        .orShould()
        .beAssignableTo(DomainAttribute.class)
        .as("All concrete domain model classes must implement DomainEntity or DomainAttribute")
        .allowEmptyShould(true);
  }

  /**
   * Every instance field declared on a {@link DomainEntity} class must be of a type that implements
   * {@link DomainAttribute}.
   *
   * <p>This enforces that entities never hold raw Java types ({@code String}, {@code int}, {@code
   * boolean}, …) directly — every value must be expressed through a named, typed domain attribute.
   */
            ...
            if (!field.getRawType().isAssignableTo(DomainAttribute.class)) {
              events.add(
                  SimpleConditionEvent.violated(
                      field,
                      String.format(
                          "Field [%s] in [%s] has type [%s] which does not implement"
                              + " DomainAttribute",
                          field.getName(),
  ...
  /**
   * Public methods on JAX-RS resource classes (annotated with {@code @Path}) must not use {@link
   * DomainEntity} or {@link DomainAttribute} types directly as parameter or return types.
   *
   * <p>Domain types are internal model classes. Exposing them directly in the HTTP API means that
   * every internal model change automatically breaks the API contract — clients would see different
   * field names, types, or structure without any deliberate versioning decision. The web layer must
   * map domain objects to dedicated DTO/response types before returning them, and must accept plain
   * Java types (e.g. {@code UUID}, {@code String}) as parameters.
   *
   * <p>Jackson serializers, DTO factory methods (e.g. {@code from(DomainType)}), and other
   * web-layer infrastructure are intentionally excluded — only {@code @Path}-annotated resource
   * classes are checked.
   *
   * <p>Note: generic type arguments (e.g. the {@code T} in {@code List<T>}) are not inspected —
   * only the raw declared types are checked.
   */
            ...
            new ArchCondition<JavaMethod>(
              ...
              public void check(final JavaMethod method, final ConditionEvents events) {
                ...
                method
                    ...
                    .forEach(paramType -> checkType(method, paramType, "parameter type", events));
              }
    ...
    return noClasses()
        .should(
            new ArchCondition<JavaClass>(
                "not use URLConnection or call URL.openStream/openConnection") {
              private static final Set<String> FORBIDDEN_CLASSES =
                  Set.of("java.net.URLConnection", "java.net.HttpURLConnection");
              private static final Set<String> FORBIDDEN_URL_METHODS =
                  Set.of("openStream", "openConnection");

              @Override
              public void check(final JavaClass clazz, final ConditionEvents events) {
                clazz.getDirectDependenciesFromSelf().stream()
                    .filter(dep -> FORBIDDEN_CLASSES.contains(dep.getTargetClass().getName()))
                    .forEach(
                        dep ->
                            events.add(
                                SimpleConditionEvent.violated(
                ...
                clazz.getMethodCallsFromSelf().stream()
                    .filter(
                        call ->
                            "java.net.URL".equals(call.getTarget().getOwner().getName())
                                && FORBIDDEN_URL_METHODS.contains(call.getTarget().getName()))
                    .forEach(
                        call ->
                            events.add(
                                SimpleConditionEvent.violated(
  ...
  /**
   * Returns all rules for a feature module. Prefer calling the individual methods when you want
