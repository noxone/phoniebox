 ...
 *
 * <p>Pause works by calling {@link SourceDataLine#stop()} which halts audio
 * output; the write loop eventually blocks when the internal buffer fills.
 * Resume calls {@link SourceDataLine#start()} which drains the buffer and
 * unblocks the write. Stop calls {@link SourceDataLine#flush()} after
 * stopping, clearing the buffer so any blocked write can complete and the
 * loop can check the {@code stopRequested} flag and exit cleanly.
 *
 * <p>Supported formats: WAV/AIFF/AU (JDK native) + MP3 (via mp3spi SPI on
 * the classpath). Any compressed format is transparently decoded to PCM.
 */
@ApplicationScoped
public class AudioApplicationService implements PlayAudioUseCase, PlaybackControlUseCase {
    ...
    private void writeLoop(final AudioInputStream audio, final SourceDataLine line) throws IOException {
    ...
    /**
     * If the stream is compressed (e.g. MP3), converts it to 16-bit signed PCM
     * so {@link SourceDataLine} can play it directly. PCM streams are returned unchanged.
     */
